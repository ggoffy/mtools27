...
class Utility extends Common\SysUtility
{
    ...
    // XOOPS表單安全檢查  XOOPS form security check
        ...
        switch ($type) {
            ...
            default:
                if (empty($ver)) {
                    ...
                    [$ver] = \is_array($_xfRow = (($xoopsDB->isResultSet($result) && ($result instanceof \mysqli_result)) ? $xoopsDB->fetchRow($result) : false)) ? $_xfRow : [null];
    ...
    //建立目錄 Create a folder
    public static function mk_dir($dir = '')
    {
        //若無目錄名稱秀出警告訊息 If no directory name is displayed, a warning message is displayed
        if (empty($dir)) {
            return;
        }

        //若目錄不存在的話建立目錄 If the directory does not exist, create a directory
        if (!\is_dir($dir)) {
            \umask(000);
            //若建立失敗秀出警告訊息 If a failed display is created, a warning message is displayed
    ...
    //路徑導覽，需搭配 get_模組_cate_path($分類編號);  Path navigation, with get_ module _cate_path (classification number)
    ...
    //  Solved the problem that basename could not catch the middle document name
    ...
    // Load bootstrap, which is currently only available in the background
    public static function get_bootstrap($mode = '')
    {
        ...
        $theme_set = (string)($xoopsConfig['theme_set'] ?? 'default');

        $sql = 'select `tt_use_bootstrap`,`tt_bootstrap_color`,`tt_theme_kind` from `' . $xoopsDB->prefix('mtools_setup') . '` where `tt_theme`=' . $xoopsDB->quote($theme_set);

        $result = $xoopsDB->query($sql);

        $row = (!$xoopsDB->isResultSet($result) || !($result instanceof \mysqli_result))
            ? false
            : (($xoopsDB->isResultSet($result) && ($result instanceof \mysqli_result)) ? $xoopsDB->fetchRow($result) : false);

        if (!\is_array($row)) {
            $row = [0, 'bootstrap3', 'bootstrap3'];
        }

        [$tt_use_bootstrap, $tt_bootstrap_color, $tt_theme_kind] = $row;
        $tt_bootstrap_color = (string)$tt_bootstrap_color;
        $tt_theme_kind      = (string)$tt_theme_kind;

        $_SESSION['theme_kind']                    = $tt_theme_kind;
        if (!isset($_SESSION[$theme_set]) || !\is_array($_SESSION[$theme_set])) {
            $_SESSION[$theme_set] = [];
        }
    ...
    //單選回復原始資料函數
    // Single-select restore the original data function
    public static function chk($DBV = null, $NEED_V = '', $defaul = '', $return = 'checked')
    {
        if ($DBV == $NEED_V) {
            return $return;
        }
    ...
    //複選回復原始資料函數
    // Select the Restore Original Data function
    ...
    public static function power_chk($perm_name = '', $perm_itemid = '', $module_id = '')
    {
        ...
        //取得群組權限功能 Get group permission features
        $gpermHandler = \xoops_getHandler('groupperm');

        //權限項目編號 Permission item ID
        $perm_itemid = (int)$perm_itemid;
        //依據該群組是否對該權限項目有使用權之判斷 ，做不同之處理
        // Different treatments will be made based on the judgment of whether the group has the right to use the permission item
        if ($gpermHandler->checkRight($perm_name, $perm_itemid, $_SESSION['groups'], $module_id)) {
            return true;
        }
    ...
    //把字串換成群組 Replace the string with a group
    ...
    //取得所有群組 Get all groups
    public static function get_all_groups(): array
    {
        global $xoopsDB;
        $sql    = 'select groupid,name from ' . $xoopsDB->prefix('groups');
        $result = $xoopsDB->query($sql);
        while (\is_array($_xfRow = (($xoopsDB->isResultSet($result) && ($result instanceof \mysqli_result)) ? $xoopsDB->fetchRow($result) : false)) && (list($groupid, $name) = $_xfRow)) {
            $data[$groupid] = $name;
        }
    ...
    //判斷字串是否為utf8 Check if the string is UTF8
    public static function is_utf8($str): int
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($sbit < 128) {
                //本字節為英文字符，不與理會 This byte is an English character and is not ignored
            } elseif ($sbit > 191 && $sbit < 224) {
                //第一字節為落於192~223的utf8的中文字(表示該中文為由2個字節所組成utf8中文字)，找下一個中文字
                // The first byte is the Chinese character of utf8 falling on 192~223 (indicating that the Chinese is a Chinese character composed of 2 bytes of utf8), find the next Chinese character
                $i++;
            } elseif ($sbit > 223 && $sbit < 240) {
                //第一字節為落於223~239的utf8的中文字(表示該中文為由3個字節所組成的utf8中文字)，找下一個中文字
                // The first byte is the Chinese character of utf8 falling in 223~239 (indicating that the Chinese is a Chinese character composed of 3 bytes), find the next Chinese character
                $i += 2;
            } elseif ($sbit > 239 && $sbit < 248) {
                //第一字節為落於240~247的utf8的中文字(表示該中文為由4個字節所組成的utf8中文字)，找下一個中文字
                // The first byte is the Chinese character of UTF8 that falls on 240~247 (indicating that the Chinese is a UTF8 Chinese character composed of 4 bytes), and find the next Chinese character
                $i += 3;
            } else {
                //第一字節為非的utf8的中文字 The first byte is the Chinese character of utf8 that is non-existent
                return 0;
            }
        }
        //檢查完整個字串都沒問體，代表這個字串是utf8中文字
        // Checking the entire string without asking the font means that this string is utf8 Chinese character
        return 1;
    }

    //轉換編碼 （_CHARSET 在後面時，$OS2Web 為 true，預設）
    //Conversion encoding (_CHARSET is true for OS2Web, default when it is behind)
    ...
    //刪除整個目錄 Delete the entire folder
    ...
    public static function toolbar_bootstrap($interface_menu = [], $force = false)
    {
        ...
        if (\is_array($interface_menu)) {
            ...
            foreach ($interface_menu as $title => $url) {
                ...
                $urlPath = (empty($moduleName) or mb_strpos($url, 'https://') === 0) ? $url : XOOPS_URL . "/modules/{$moduleName}/{$url}";
    ...
    public static function mobile_device_detect($iphone = true, $ipad = true, $android = true, $opera = true, $blackberry = true, $palm = true, $windows = true, $mobileredirect = false, $desktopredirect = false)
    {
        ...
        // header('Cache-Control: no-transform'); // https://mobiforge.com/developing/story/setting-http-headers-advise-transcoding-proxies
        // header('Vary: User-Agent, Accept'); // https://mobiforge.com/developing/story/setting-http-headers-advise-transcoding-proxies
